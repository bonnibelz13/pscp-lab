"""Ratatype"""


def main(num):
    """num 1-4 print text in the pic"""
    if num == 1:
        print("In Deep Learning, a Convolutional Neural Network \
(CNN) is a class of Deep Neural Networks, most commonly applied \
to analyzing visual imagery.")
    elif num == 2:
        print("\"Two things are infinite: the universe and human stupidity; \
and I'm not sure about the universe\". \
- Albert Einstein")
    elif num == 3:
        print("Statistics is the discipline that concerns the collection, \
organization, displaying, analysis, interpretation and presentation of data.")
    elif num == 4:
        print("The backslash (\\) is a typographical mark \
used mainly in computing and is the mirror image of the common slash (/). \
It is sometimes called \"escape character\".")
main(num=int(input()))
